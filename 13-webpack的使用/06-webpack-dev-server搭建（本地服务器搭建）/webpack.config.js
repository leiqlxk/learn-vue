...
module.exports = {
  ...
  //webpack提供了一个可选的本地开发服务器，这个本地服务器基于node.js搭建，可以实现我们想要的让浏览器自动刷新显示我们修改后的结果
  //这种方式就直接把代码加载到内存中运行，并不会生成dist文件，速度更快
  //它是一个单独的模块，在使用之前必须先安装，npm install webpack-dev-server --save-dev
  //devserver也是作为webpack中的一个选项，选项本身可以设置如下属性：
  //contentBase：为哪一个文件夹提供本地服务，默认是根文件夹，我们这里填写./dist
  //port：端口号
  //inline：页面实时刷新
  //historyApiFallback：在SPA页面中，依赖HTML5的history模式
  //配置完成后在package.json中新建一个指令'dev': 'webpack-dev-server --open'
  devServer: {
    contentBase: './dist',
    //默认8080端口
    // port: 80,
